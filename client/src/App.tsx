...
const APK_URL = "./downloads/sougxpress.apk";
  ...
  return (
    <div className="site-shell">
      ...
      <main id="top">
        ...
        <section className="cycle-section section-pad" aria-labelledby="cycle-title"><div className="container"><div className="section-heading cycle-heading"><div className="eyebrow eyebrow-dark"><span className="eyebrow-dot" /> الدورة التجارية</div><h2 id="cycle-title">كل طرف يعرف<br /><span>خطوته التالية.</span></h2></div><div className="cycle-route"><div className="cycle-line" />{[{ label: "التاجر", icon: Store }, { label: "المنتجات", icon: PackageCheck }, { label: "الزبون", icon: Users }, { label: "الطلب", icon: Smartphone }, { label: "التجهيز", icon: ShieldCheck }, { label: "التوصيل", icon: Truck }, { label: "باب الزبون", icon: HeartHandshake }].map(({ label, icon: Icon }, index) => <div className={`cycle-node cycle-node-${index + 1}`} key={label}><span className="cycle-icon"><Icon size={20} /></span><strong>{label}</strong></div>)}</div><p className="cycle-caption">التاجر يعرض، الزبون يختار، الطلب يتحرك، والباب يستقبل.</p></div></section>
